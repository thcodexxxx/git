...
- SAN属性を含む要求（Webサーバ証明書）を行う場合、エンタープライズCA側で設定の変更（`certutil -setreg policy\EditFlags +EDITF_ATTRIBUTESUBJECTALTNAME2` およびサービスの再起動）が必要です。これを実施していない場合、要求が拒否されるか、SANが含まれない証明書が発行されます。
- PFXファイルにはパスワード保護がかかっていない秘密鍵情報が含まれます。Output フォルダやファイルの取り扱い・保管には十分注意してください。
